...
def type_and_enter(text):
    if arguments[1] == 'k' or arguments[1] == 'km':
        ...
        return
    if arguments[2] == 'directinput':
        pydirectinput.typewrite(str(text), interval=.1)
...
def step_2():
    ...
    if window_mode == "full":
        w, h = im.size
        expanded = Image.new('RGB', (w+6, h+29))
        expanded.paste(im, (3, 26, w+3, h+26))
        im = expanded
